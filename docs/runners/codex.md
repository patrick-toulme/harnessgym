...
!!! note "Non-git workspaces"
    HarnessGym always passes `--skip-git-repo-check` to `codex exec`, because
    `codex` (≥ 0.142) otherwise refuses to run outside a trusted git directory
    and task workspaces are frequently plain folders. You don't need to add it
    to your `harnessgym run` commands — the runner injects it for you.
