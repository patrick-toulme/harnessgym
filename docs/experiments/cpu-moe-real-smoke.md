# CPU MoE HarnessGym Real Smoke

Workspace copy:

```text
tmp/cpu_moe_real_smoke
```
...
```bash
harnessgym run \
  --task tmp/cpu_moe_real_smoke/task.md \
  --workspace tmp/cpu_moe_real_smoke \
  --iterations 2 \
  --attempt-timeout 5m \
...
- Run artifacts: `tmp/cpu_moe_real_smoke/.harnessgym/runs/real-smoke`
