...
```bash
rm -rf tmp/h100_triton_harness_long
mkdir -p tmp/h100_triton_harness_long
cp -R examples/triton_rmsnorm_h100_task/. tmp/h100_triton_harness_long/
cp -R examples/triton_rmsnorm_h100_harness_artifacts/.harnessgym tmp/h100_triton_harness_long/
...
python3 -m harnessgym.cli run \
  --task tmp/h100_triton_harness_long/task.md \
  --workspace tmp/h100_triton_harness_long \
  --iterations 4 \
  --attempt-timeout 8m \
...
```text
tmp/h100_triton_harness_long/.harnessgym/runs/<run-id>/
```
...
```bash
cd tmp/h100_triton_harness_long
