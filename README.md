...
Codex session. See `docs/experiments/h100-triton-rmsnorm.md` and
...
`docs/experiments/h100-triton-rmsnorm-long.md`.
...
- Report path: `tmp/c_flash_attention_compare_deep_tests_2turn/compare_report.json`.
...
harnessgym compare \
  ...
  --task task.md \
  --artifact-source tmp/cpu_attention_autotune_harness5/.harnessgym \
  --output-dir tmp/cpu_attention_autotune_compare_pg \
  --trials 1 \
  --iterations 5 \
  --attempt-timeout 2m \
  --runner exec \
...
- Report path: `tmp/cpu_attention_autotune_compare_pg/compare_report.json`.
...
- Report path: `tmp/cpp_stencil_compare_hardened_1iter/compare_report.json`.
