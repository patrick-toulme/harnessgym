...
class ExecRunner(Runner):
    ...
    def build_command(
        ...
        # `codex exec` (>= 0.142) refuses to run outside a trusted git directory
        # unless --skip-git-repo-check is passed, so HarnessGym always passes it;
        # workspaces are frequently plain (non-git) task directories.
        base = [self.codex_bin, "exec", "--skip-git-repo-check"]
        if session_id:
            return [*base, "resume", *config_overrides, session_id, prompt]
        return [*base, *config_overrides, prompt]
